...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  Future<void> _loadSchedule() async {
    ...
    if (jsonStr != null) {
      ...
      setState(() {
        schedule = data.map((e) => ScheduleItem.fromJson(e)).toList();
      });
    } else {
      // If no saved schedule exists, load the default from CSV
  ...
  Future<void> _playBellSound() async {
    try {
      await _audioPlayer.play(AssetSource('bell-meditation-75335.mp3'));
    } catch (e) {
  ...
  void _editItem(int index) {
    ...
    try {
      ...
      DateTime parsedEndTime = DateFormat(
        'h:mm a',
        'en_US',
      ).parseStrict(item.endTime);

      final endTime = parsedEndTime;
      final durationInMinutes = endTime.difference(parsedTime).inMinutes;
